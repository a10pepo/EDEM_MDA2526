...
CONF_CONSUMER = {
    ...
    "group.id": "equipo_uci_v1",
    "auto.offset.reset": "earliest",
}

TOPIC_ALERTAS = "alertas_uci_topic"
...
def pintar_alerta_uci(datos):
    ...
    fc = get_field(datos, "frecuencia_cardiaca", "FRECUENCIA_CARDIACA")
    tas = get_field(datos, "tension_sistolica", "TENSION_SISTOLICA")
    tad = get_field(datos, "tension_diastolica", "TENSION_DIASTOLICA")
    spo2 = get_field(datos, "saturacion_oxigeno", "SATURACION_OXIGENO")
    temp = get_field(datos, "temperatura", "TEMPERATURA")
    ...
    print(
        Fore.RED
        + Style.BRIGHT
        + f"🚨 [UCI][{momento}] INGRESO INMEDIATO | Paciente {paciente_id} (hab. {habitacion})\n"
...
def main():
    ...
    print(
        Fore.CYAN
        + Style.BRIGHT
        + f"📡 [UCI] Escuchando alertas críticas en topic {TOPIC_ALERTAS}"
    )
    ...
    try:
        while True:
            ...
            if msg is None:
                continue
            if msg.error():
                print(Fore.RED + f"❌ Error consumer UCI: {msg.error()}")
                continue

            datos = json.loads(msg.value().decode("utf-8"))
            pintar_alerta_uci(datos)

    except KeyboardInterrupt:
        print(Fore.YELLOW + "\n🛑 Consumer UCI interrumpido.")
    finally:
        consumer.close()
        print(Fore.CYAN + "✅ Consumer UCI detenido.")
