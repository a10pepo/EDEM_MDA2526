...
CONF_CONSUMER = {
    ...
    "group.id": "equipo_seguimiento_v1",
    "auto.offset.reset": "earliest",
}

TOPIC_ALERTAS = "alertas_seguimiento_topic"
...
def pintar_alerta_seguimiento(datos):
    ...
    print(
        Fore.CYAN
        + Style.BRIGHT
        + f"ℹ️ [SEGUIMIENTO][{momento}] Control periódico | Paciente {paciente_id} (hab. {habitacion})\n"
...
def main():
    ...
    print(
        Fore.CYAN
        + Style.BRIGHT
        + f"📡 [SEGUIMIENTO] Escuchando pacientes estables de alto riesgo en {TOPIC_ALERTAS}"
    )
    ...
    try:
        while True:
            ...
            if msg is None:
                continue
            if msg.error():
                print(Fore.RED + f"❌ Error consumer Seguimiento: {msg.error()}")
                continue

            datos = json.loads(msg.value().decode("utf-8"))
            pintar_alerta_seguimiento(datos)

    except KeyboardInterrupt:
        print(Fore.YELLOW + "\n🛑 Consumer Seguimiento interrumpido.")
    finally:
        consumer.close()
        print(Fore.CYAN + "✅ Consumer Seguimiento detenido.")
