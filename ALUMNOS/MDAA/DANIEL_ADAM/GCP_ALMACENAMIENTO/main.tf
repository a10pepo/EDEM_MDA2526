...
resource "google_compute_instance_from_machine_image" "delivery_app" {
  ...
  zone           = var.zone
  source_machine_image  = "projects/${var.project_id}/global/machineImages/${var.machine_image}"
...
## Dar permisos de BigQueery a la service account para crear el SUB que falla
...
### Añado esto para crear el bucket que me pide para el datalake (el comando nohup)

resource "google_storage_bucket" "datalake_bucket" {
  name          = "datalake-orders-${var.project_id}" 
  location      = var.region
  force_destroy = true  
